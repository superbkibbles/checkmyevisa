...
const WelcomePage = () => {
    ...
    const listCountries = ["Afghanistan",
        "Albania",
        "Algeria",
        "American Samoa",
        "Andorra",
    ...
    const changeHandler = (e) => {
        const v = e.target.value.replace(new RegExp("\\\\", "g"), "");
        setSelectedCountry(v)
        const c = listCountries.filter((country) => country.toLowerCase().search(v.toLowerCase()) !== -1)
        if ( c.length > 0 )
    ...
    const listClick = (country) => {
        setSelectedCountry(country)
        setFilteredCountries([])
    }
    return (
        <div className="welcome" onClick={() => setFilteredCountries([])}>
